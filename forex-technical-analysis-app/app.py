...
st.title("Forex Technical Analysis Dashboard")
st.write("Real-time forex and gold charts from TradingView plus AI trading signals.")
...
with st.sidebar:
    ...
    period = st.selectbox("Period", ["7d", "1mo", "3mo", "6mo", "1y", "2y", "5y", "max"], index=1)
    interval = st.selectbox("Interval", ["5m", "15m", "1h", "1d", "1wk"], index=1)
...
def predict_next_price(df: pd.DataFrame, lookback: int = 12) -> tuple[float, str, float]:
    """Predict the next close price using a simple linear regression on recent closes."""
    closes = df["Close"].dropna()
    if len(closes) < 5:
        return float(closes.iloc[-1]), "Insufficient history", 0.0
...
def get_ai_signal(df: pd.DataFrame) -> tuple:
    """Advanced AI trading signal with weighted indicators"""
...
col2.metric("Last Close", f"{latest_close:.5f}")
col3.metric("Change", f"{price_change:.5f} ({price_change_pct:.2f}%)")
col4.metric("RSI (14)", f"{latest['RSI_14']:.2f}")

if enable_ai and interval == "15m":
    st.subheader("🤖 Advanced AI Trading Signal (15m)")
    signal, confidence, status, explanation = get_ai_signal(df)
    ...
    for i, part in enumerate(analysis_parts):
        cols[i % 2].info(part)
elif enable_ai and interval != "15m":
    st.warning("⏰ AI analysis is optimized for 15m interval. Switch to 15m for trading signals.")

st.subheader("Live TradingView Chart")
st.caption("This embedded widget loads the live market chart for the selected pair or gold.")

if interval == "5m":
    try:
        next_price, next_direction, next_change_pct = predict_next_price(df, lookback=15)
        st.subheader("🔮 5-Minute Price Prediction")
        st.metric(
            "Predicted Close in 5m",
            f"{next_price:.5f}",
            f"{next_direction} {next_change_pct:+.2f}%",
        )
        st.caption("Prediction based on linear regression of the latest 5-minute bars.")
    except Exception as exc:
        st.warning(f"Unable to calculate 5-minute prediction: {exc}")

tradingview_symbol = quote(SYMBOL_MAP[symbol], safe="")
tradingview_url = (
    "https://s.tradingview.com/widgetembed/?frameElementId=tradingview_chart"
    f"&symbol={tradingview_symbol}"
    f"&interval={INTERVAL_MAP[interval]}"
    "&theme=dark"
    "&style=1"
    "&locale=en"
    "&toolbarbg=f1f3f6"
...
st.iframe(tradingview_url, height=720)
st.link_button("Open this chart in TradingView", tradingview_url)
...
if show_bbands:
    fig.add_trace(go.Scatter(x=df["Date"], y=df["BB_UPPER"], mode="lines", name="BB Upper", line=dict(dash="dot")), row=1, col=1)
    fig.add_trace(go.Scatter(x=df["Date"], y=df["BB_LOWER"], mode="lines", name="BB Lower", line=dict(dash="dot")), row=1, col=1)

fig.add_trace(go.Bar(x=df["Date"], y=df["Volume"], name="Volume", marker_color="#7f8c8d"), row=2, col=1)

if show_rsi:
    fig.add_trace(go.Scatter(x=df["Date"], y=df["RSI_14"], mode="lines", name="RSI 14"), row=2, col=1)
    fig.add_hline(y=70, line_dash="dash", line_color="red", row=2, col=1)
    fig.add_hline(y=30, line_dash="dash", line_color="green", row=2, col=1)

fig.update_layout(height=700, margin=dict(l=10, r=10, t=40, b=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
st.plotly_chart(fig, width="stretch")

st.subheader("Signal Summary")
if latest_close > latest["SMA_20"] and latest["RSI_14"] > 50:
    signal_bias = "Bullish"
elif latest_close < latest["SMA_20"] and latest["RSI_14"] < 50:
    signal_bias = "Bearish"
else:
    signal_bias = "Neutral"
...
latest_rows = latest_rows[["Date", "Open", "High", "Low", "Close", "Volume", "SMA_20", "SMA_50", "RSI_14"]]
